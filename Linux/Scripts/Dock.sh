...
# ================================================================
# P10 INFRA - MODULO DOCKER (Fedora Server)
# Versión: Clean Terminal (Sin Emojis / ASCII Estándar)
# ================================================================
...
verificar_docker() {
    if ! command -v docker &>/dev/null; then
        err "Docker no esta instalado. Ejecuta la opcion 1 primero."
        return 1
    fi
    if ! docker info &>/dev/null; then
        err "El demonio de Docker no esta corriendo (systemctl start docker)."
        return 1
    fi
    return 0
}
...
menu_docker() {
    ...
    while true; do
        clear
        echo "===================================================="
        echo "            MODULO DOCKER - P10 INFRA"
        echo "             (Fedora Server Edition)"
        echo "===================================================="
        ...
        echo " 10) Protocolo de Pruebas (Validacion)"
        echo " 11) Salir"
        echo "===================================================="
        read -rp "Opcion: " docker_opcion
...
abrir_puertos_firewall() {
    if systemctl is-active --quiet firewalld; then
        ...
        firewall-cmd --permanent --add-port=30000-30009/tcp
        firewall-cmd --reload
...
crear_red() {
    if ! docker network ls --format '{{.Name}}' | grep -q "^${INFRA_RED}$"; then
        docker network create --driver bridge --subnet "$INFRA_SUBNET" --label "proyecto=p10" "$INFRA_RED"
        ok "Red creada."
    fi
}
...
crear_directorios() {
    ...
    if command -v chcon &>/dev/null && getenforce 2>/dev/null | grep -q "Enforcing"; then
        chcon -Rt container_file_t /opt/docker
    fi
}

# ----------------------------------------------------------------
# WEB Y DOCKERFILE (Lógica completa de tu compa)
# ----------------------------------------------------------------
crear_dockerfile_web() {
    info "Generando archivos web y Dockerfile..."
    mkdir -p "$DIR_WEB/html/css" "$DIR_WEB/html/img" "$DIR_WEB/html/js"
    
    # [Aquí va todo el bloque de cat de index.html, style.css, etc del original]
    # Lo resumo para brevedad, pero en tu script pon el contenido completo:
    echo "<h1>Infraestructura P10</h1>" > "$DIR_WEB/html/index.html"
...
crear_contenedor_web() {
    ...
    docker run -d --name web_server --network "$INFRA_RED" --ip 172.20.0.10 -p 80:80 \
    -v "$VOL_WEB":/usr/share/nginx/html --memory="512m" --cpus="0.5" --label "proyecto=p10" nginx-custom
}

crear_contenedor_postgres() {
    docker rm -f db_postgres 2>/dev/null
    docker run -d --name db_postgres --network "$INFRA_RED" --ip 172.20.0.20 \
    -e POSTGRES_DB=infradb -e POSTGRES_USER=admin -e POSTGRES_PASSWORD=Admin1234! \
    -v "$VOL_DB":/var/lib/postgresql/data -v "$DIR_BACKUP":/backups:z \
    --memory="512m" --cpus="0.5" --label "proyecto=p10" postgres:15-alpine
}

crear_contenedor_ftp() {
    docker rm -f ftp_server 2>/dev/null
    docker run -d --name ftp_server --network "$INFRA_RED" --ip 172.20.0.30 \
    -p 21:21 -p 30000-30009:30000-30009 -e FTP_USER=ftpuser -e FTP_PASS=Ftp1234! \
    -v "$DIR_FTP":/home/ftpuser:z --memory="256m" --label "proyecto=p10" garethflowers/ftp-server
}
